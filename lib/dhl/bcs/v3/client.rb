...
module Dhl::Bcs::V3
  class Client
    ...
    def initialize(config, log: true, test: false, **options)
      ...
      @client = if ENV['SAVON_USE_REMOTE_WSDL'].nil?
        client_with_local_wsdl(config)
      else
        client_with_remote_wsdl(config)
      end
    end
    ...
    def client_with_remote_wsdl(config)
      Savon.client(base_client_options(config).merge(wsdl: WSDL))
    end
    ...
    def base_client_options(config)
      {
